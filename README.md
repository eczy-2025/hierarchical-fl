根据我们目前实现的**动态招募**、**随机掉线模拟**以及**死锁修复**等高级特性，以下是更新后的 `README.md` 内容。

主要更新点包括：

1. **流程描述**：增加了Edge端的“招募窗口”和Client端的“随机报名”机制。
2. **配置说明**：补充了 `online_rate`, `recruitment_time`, `min_clients` 等新参数。
3. **预期日志**：更新为反映动态拓扑和鲁棒性测试的日志样例。
